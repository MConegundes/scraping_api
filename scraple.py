...
def scraple_books():
    ...
    print("Links de coleção por categoria coletados com sucesso!")
    ...
    for link in links_of_categories:
        ...
        counter += 1 
        print (f"category {counter}/{len(links_of_categories)} scraped successfully!")

    books_df = pd.DataFrame(books_scraped)
    books_df.to_csv('books_data.csv', index_label='index')

    return {"message": "Books scraped successfully!"}
